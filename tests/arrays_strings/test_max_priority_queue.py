from nose.tools import assert_equal
from data_types.max_priority_queue import PriorityQueueNode, MaxPriorityQueue

class TestMaxPriorityQueue(object):

    def test_max_priority_queue(self):
        priority_queue = MaxPriorityQueue()
        assert_equal(priority_queue.extract_max(), None)
        priority_queue.insert(PriorityQueueNode('a', 20))
        priority_queue.insert(PriorityQueueNode('b', 5))
        priority_queue.insert(PriorityQueueNode('c', 15))
        priority_queue.insert(PriorityQueueNode('d', 22))
        priority_queue.insert(PriorityQueueNode('e', 40))
        priority_queue.insert(PriorityQueueNode('f', 3))
        #priority_queue.decrease_key('f', 2)
        #priority_queue.decrease_key('a', 19)
        mins = []
        while priority_queue.array:
            mins.append(priority_queue.extract_max().key)
        assert_equal(mins, [40, 22, 20, 15, 5, 3])
        print('Success: test_min_heap')


def main():
    test = TestMinPriorityQueue()
    test.test_min_priority_queue()

if __name__ == '__main__':
    main()